...
const BookDetail = () => {
	...
	function updateCart() {
		...
		save({ item_book: itembooks, quantity: numbers, user: localStorage.getItem("userId") });
		localStorage.setItem("number_items", parseInt(localStorage.getItem("number_items")) + 1);
	}
	async function save(data) {
		addCart(localStorage.getItem("access_token"), data)
			.then(response => {
				// console.log(response);
				// getCart();
	...
	async function getCart() {
		getCarts(localStorage.getItem("access_token"), localStorage.getItem("userId"))
			.then(response => {
				// console.log(response)
