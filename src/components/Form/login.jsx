...
const LoginForm = () => {
  ...
  async function send(data) {
    ...
    login(data)
      .then((response) => {
        ...
        if (response?.data?.user?.role?.name === "admin") history("/admin");
        else history("/");
  ...
  const { handleChange, handleSubmit, clearForm, values, errors } = useForm(
    validate,
    send,
    { username: "", password: "" }
  );
            ...
            <div className="flex flex-wrap justify-between mt-4">
              <div className="w-full " onClick={LoginFace}>
                <div className="flex items-center border-solid justify-center border-2 border-green py-1 rounded-md cursor-pointer sm:mr-2">
                  <img src={`images/icons/facebook.png`} className="h-7" />
            ...
            <div className="flex justify-center">
              <div className="inline-flex items-center p-2 rounded-lg shadow-custom">
                <img
                  ...
                  alt=""
                  className="contain h-8 w-8"
                />
                <p className="ml-2 font-medium text-sm md:text-base">
                  Successfully
                </p>
              </div>
            </div>
